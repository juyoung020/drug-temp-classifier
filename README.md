...
이 시스템은 **온톨로지가 먼저 추론하고, LLM이 그 결과를 참조해 최종 판단**하는 구조입니다.  
...
| HSV 색상 추출 | 지배 색상 판별 |
| OWL 온톨로지 | 신호 스코어링 → Symbolic Scene Representation 생성 → 온도 추론 |
| Gemma4 LLM | 이미지 + 온톨로지 맥락 참조 → 최종 계열 판단 |
...
LLM은 이 구조화된 근거를 보고 최종 판단하므로, 내부 추론(`think`)을 끄고도 정확도를 유지합니다.
...
| CLIP 형태 인식 | 1초 미만 |
| HSV 색상 추출 | 약 0.1초 |
| OWL 추론 (장면 표현 생성) | 약 0.1초 미만 |
| **Gemma4 LLM** ← 병목 | **약 3\~10초** |
| OWL 추론 (온도 도출 · HermiT) | 약 0.5\~1초 |
| **총합** | **약 5\~14초** |
